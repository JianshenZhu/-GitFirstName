...
class Signal{
    ...
    void offset(float *p);
    void scale(float *p);
    void newoffset(float *p);
    void newscale(float *p);
    void Sig_info();
...
void Signal::operator*(double &fa)
{	
	for (int i = 0; i < size; i++){
	*(p+i) = *(p+i) * fa;
}
...
void Signal::Sig_info(){//used to cout info
                        ...
                        else
                {
                    ...
                    cout << length << endl;
                    size=length;
                    q=(float*)malloc((size)*sizeof(int));
                    while (fscanf(fop, "%f", &w) != EOF) {//read the second value of the file which is the maximum possible value.
		            for (i=0;i<size;++i){
		            fscanf(fop, "%f", (q+i));//the for loop is to read all the values in the file.
	                   }
                    ave(q);//calculate mean value.
                    mav(q);//find the max value.
            }
        }
}
